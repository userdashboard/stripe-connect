...
function convertOwner (e) {
  ...
  var beneficialOwner = {
    ...
    id_number: document.getElementById('relationship_owner_personal_id_number').value,
    address: {
  ...
  return window.uploadDocumentFiles(documentFront, documentBack, function (error, front, back) {
    ...
    if (front && front.id) {
      ...
      return window.renderError('invalid-relationship_owner_verification_document_front')
    }
    if (back && back.id) {
      ...
      return window.renderError('invalid-relationship_owner_verification_document_back')
    }
