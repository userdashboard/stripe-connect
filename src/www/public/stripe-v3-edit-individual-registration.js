...
function updateAccount (e) {
  ...
  var firstNameKanji = document.getElementById('first_name_kanji')
  if (firstNameKanji) {
    ...
    var lastNameKanji = document.getElementById('last_name_kanji')
    ...
    var firstNameKana = document.getElementById('first_name_kana')
    ...
    var lastNameKana = document.getElementById('last_name_kana')
  ...
  if (addressLine1 && addressLine1.value) {
    ...
    var postalCode = document.getElementById('address_postal_code')
    if (postalCode && postalCode.value) {
      accountData.individual.address.postal_code = postalCode.value
    }
  }
  var line1Kana = document.getElementById('address_kana_line1')
  if (line1Kana) {
    if (!line1Kana.value || !line1Kana.value.length) {
      return window.renderError('invalid-address_kana_line1')
    }
    accountData.individual.address_kana = {
      line1: line1Kana.value
    }
    var cityKana = document.getElementById('address_kana_city')
    if (cityKana.value && cityKana.value.length) {
      accountData.individual.address_kana.city = cityKana.value
    } else {
      return window.renderError('invalid-address_kana_city')
    }
    var stateKana = document.getElementById('address_kana_state')
    if (stateKana.value && stateKana.value.length) {
      accountData.individual.address_kana.stateKana = stateKana.value
    } else {
      return window.renderError('invalid-address_kana_state')
    }
    var townKana = document.getElementById('address_kana_town')
    if (townKana.value && townKana.value.length) {
      accountData.individual.address_kana.town = townKana.value
    } else {
      return window.renderError('invalid-address_kana_town')
    }
    var postalCodeKana = document.getElementById('address_kana_postal_code')
    if (postalCodeKana.value && postalCodeKana.value.length) {
      accountData.individual.address_kana.postal_code = postalCodeKana.value
    } else {
      return window.renderError('invalid-address_kana_postal_code')
    }
    var line1Kanji = document.getElementById('address_kanji_line1')
    if (!line1Kanji.value || !line1Kanji.value.length) {
      return window.renderError('invalid-address_kanji_line1')
    }
    accountData.individual.address_kanji = {
      line1: line1Kanji.value
    }
    var cityKanji = document.getElementById('address_kanji_city')
    if (cityKanji.value && cityKanji.value.length) {
      accountData.individual.address_kanji.city = cityKanji.value
    } else {
      return window.renderError('invalid-address_kanji_city')
    }
    var stateKanji = document.getElementById('address_kanji_state')
    if (stateKanji.value && stateKanji.value.length) {
      accountData.individual.address_kanji.stateKanji = stateKanji.value
    } else {
      return window.renderError('invalid-address_kanji_state')
    }
    var townKanji = document.getElementById('address_kanji_town')
    if (townKanji.value && townKanji.value.length) {
      accountData.individual.address_kanji.town = townKanji.value
    } else {
      return window.renderError('invalid-address_kanji_town')
    }
    var postalCodeKanji = document.getElementById('address_kanji_postal_code')
    if (postalCodeKanji.value && postalCodeKanji.value.length) {
      ...
      return window.renderError('invalid-address_kanji_postal_code')
    }
  }
